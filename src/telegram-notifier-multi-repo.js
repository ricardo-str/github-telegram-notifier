...
class TelegramNotifier {
  ...
  getRepoPrefix() {
    return this.repoIdentifier ? `[${this.repoIdentifier}] ` : '';
  }
  ...
  formatAssignmentMessage(payload) {
    ...
    if (action === 'assigned' && assignee) {
      ...
      return `${repoPrefix}[Auto-asignación de Tareas] @${assigneeName} - ${taskTitle}`;
    }
    return null;
  }
  ...
  formatProjectMessage(payload) {
    // Solo procesar eventos de project board en web-ui (sin REPO_IDENTIFIER)
    if (this.repoIdentifier) {
      console.log('Ignorando evento de project board en', this.repoIdentifier);
      return null;
    }
  ...
  async processWebhook() {
    try {
      ...
      console.log('Repositorio:', this.repoIdentifier || 'UI');
